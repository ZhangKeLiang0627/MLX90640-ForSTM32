 ...
 * â€˜freqCntâ€? argument should be changed â€? lower value results in 
 * higher frequency.
 */
...
#define SWI2C_SCL_Pin       GPIO_Pin_10

#define SWI2C_SDA_GPIO_Port GPIOB
#define SWI2C_SDA_Pin       GPIO_Pin_3

#define SCL_HIGH 	GPIO_WriteBit(SWI2C_SCL_GPIO_Port,SWI2C_SCL_Pin,GPIO_PIN_SET) //PB10
#define SCL_LOW  	GPIO_WriteBit(SWI2C_SCL_GPIO_Port,SWI2C_SCL_Pin,GPIO_PIN_RESET)

#define SDA_HIGH 	GPIO_WriteBit(SWI2C_SDA_GPIO_Port,SWI2C_SDA_Pin,GPIO_PIN_SET) //PB3
#define SDA_LOW  	GPIO_WriteBit(SWI2C_SDA_GPIO_Port,SWI2C_SDA_Pin,GPIO_PIN_RESET)

#define SDA_IN   	{SWI2C_SDA_GPIO_Port->MODER&=~(3<<(3*2));SWI2C_SDA_GPIO_Port->MODER|=0<<3*2;} //PB3
#define SDA_OUT  	{SWI2C_SDA_GPIO_Port->MODER&=~(3<<(3*2));SWI2C_SDA_GPIO_Port->MODER|=1<<3*2;}

#define sda      	GPIO_ReadInputDataBit(SWI2C_SDA_GPIO_Port,SWI2C_SDA_Pin) //PB3
...
{   
    ...
    GPIO_InitStructure.GPIO_Pin = SWI2C_SCL_Pin;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
    GPIO_Init(SWI2C_SCL_GPIO_Port, &GPIO_InitStructure);

    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_OType = GPIO_OType_OD;
    GPIO_InitStructure.GPIO_Pin = SWI2C_SDA_Pin;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
    GPIO_Init(SWI2C_SDA_GPIO_Port, &GPIO_InitStructure);

    GPIO_WriteBit(SWI2C_SCL_GPIO_Port,SWI2C_SCL_Pin,GPIO_PIN_SET); //PB10
	GPIO_WriteBit(SWI2C_SDA_GPIO_Port,SWI2C_SDA_Pin,GPIO_PIN_SET); //PB3
...
int MLX90640_I2CWrite(uint8_t slaveAddr, uint16_t writeAddress, uint16_t data)
{
    ...
    printf("dataCheck:%d, data:%d\r\n", (uint8_t)dataCheck, (uint8_t)data);
